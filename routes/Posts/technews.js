...
app.set('views', path.join(__dirname, '..', '..' , 'views' , 'Tech-news'));
app.set('view engine', 'ejs');

router.post('/mupload', upload.single('technewsfile'), async (req, res) => {
  const post = await technewsModel.find();
  let posts = post.length;
  let pages = posts / 24
  let page = Math.round(pages)
  try {
    const created = technewsModel.create({
      postname: req.body.technewstext,
      postheading: req.body.technewsheading,
    ...
    if (posts % 24 === 0 && page >= 2) {
      ...
      const filePath = path.join(__dirname, '..', '..' , 'views' , 'technews' , fileName);
      const filecontent =
        `<!DOCTYPE html>
    <html>
    <head>
    <%- include('../Template-Engine/technews1') %>
    </head>
    ...
    <body>
      ...
      <span class="present-page">page<%= PostPerPage %></span>
        <h2 class="text-center m-tb heading-color">technews</h2>
  ...
  res.redirect('/categories/technews')
});

router.get('/categories/technews', async function (req, res, next) {
  try {
    let posts = await technewsModel.find()
    const finalPosts = posts.slice(-24)
    let post = 24

    res.render('technews/technews', { finalPosts, post });
  } catch (error) {
    res.render('error', { error });
  }
});
router.get('/technews/:slug', async function (req, res, next) {
  try {
    ...
    res.render(`technews/${req.params.slug}`, { finalPosts, post, PostPerPage, roundednumber });
  } catch (error) {
    res.render('error', { error });
  }
});
